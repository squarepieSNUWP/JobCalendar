...
export default function ReviewList() {
    ...
    return (
        <Layout>
            <h1 className="text-3xl place-self-start font-bold text-left mb-9 mt-2 pl-4 text-primary">Review</h1>
            {jobs.map((j, index) => (
                <div className="w-full p-0 m-0 flex flex-col">
                    <div onClick={() => router.push(`/review/${j.id}`)} key={index} className="
                    w-full h-full rounded-3xl btn-hover color-1 px-8 py-6 cursor-pointer relative">
